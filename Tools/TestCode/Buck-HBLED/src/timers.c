...
void PWM_Configure_For_SMPS(void){
	// Set up PTE31 to use for SMPS with TPM0 Ch 4 (buck driver)
	SIM->SCGC5 |= SIM_SCGC5_PORTE_MASK;
	PORTE->PCR[31]  &= PORT_PCR_MUX(7);
	PORTE->PCR[31]  |= PORT_PCR_MUX(3);

	// Set up PTA5 to use for SMPS with TPM0 Ch 2 (timing reference)
	SIM->SCGC5 |= SIM_SCGC5_PORTE_MASK;
	PORTA->PCR[5]  &= PORT_PCR_MUX(7);
	PORTA->PCR[5]  |= PORT_PCR_MUX(3);

	// Configure TPM peripheral for buck converter
...
void PWM_Init(TPM_Type * TPM, uint8_t channel_num, uint16_t period, uint16_t duty)
{
	...
	//set TPM enable DMA transfers on overflow, count up-down and divide by 1 prescaler and clock mode
	// To Do: set TPM_SC_DMA only with mode 4?
	TPM->SC = TPM_SC_DMA(1) | TPM_SC_CPWMS(1) | TPM_SC_PS(0);
...
#if OUTPUT_TPM_TIMING_REFERENCE	
	//set channel to center-aligned high-true PWM
	TPM->CONTROLS[TPM_REF_CHANNEL].CnSC = TPM_CnSC_MSB_MASK | TPM_CnSC_ELSB_MASK;
	// Set initial duty cycle
	TPM->CONTROLS[TPM_REF_CHANNEL].CnV = period-1;
#endif
	...
	// Configure NVIC 
	NVIC_SetPriority(TPM0_IRQn, 0); // 0, 1, 2 or 3
...
void PWM_Set_Value(TPM_Type * TPM, uint8_t channel_num, uint16_t value) {
	TPM->CONTROLS[channel_num].CnV = PWM_PERIOD - value + 1;
}
extern void Control_HBLED(void);

#if USE_TPM0_INTERRUPT
void TPM0_IRQHandler() {
	static uint32_t control_divider = CTL_FREQ_DIV_FACTOR;
	
	DEBUG_START(DBG_IRQ_TPM);
	//clear pending IRQ flag
	TPM0->SC |= TPM_SC_TOF_MASK; 

	control_divider--;
	if (control_divider == 0) {
		control_divider = CTL_FREQ_DIV_FACTOR;
		// Start conversion
	...
	DEBUG_STOP(DBG_IRQ_TPM);
}
#endif // USE_TPM0_INTERRUPT
...
void Init_PIT(unsigned period) {
	...
	NVIC_SetPriority(PIT_IRQn, 3); // 0, 1, 2 or 3
