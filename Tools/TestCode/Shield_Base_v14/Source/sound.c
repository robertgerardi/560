...
uint16_t SoundBuffer[2][NUM_WAVEFORM_SAMPLES];
...
void Play_Waveform_with_DMA(void) {
	Configure_DMA_For_Playback(SoundBuffer[0], SoundBuffer[1], NUM_WAVEFORM_SAMPLES, 1);
	Start_DMA_Playback();
}
 ...
 void Thread_Refill_SoundBuffer(void * arg) {
	...
	while (1) {
		...
		if (events & EV_REFILL_SOUND_BUFFER) { 
			...
			// One or more voices has been added, so update remaining buffer
			i = Get_DMA_Transfers_Completed(0);
			if ((i < 0) || (i > NUM_WAVEFORM_SAMPLES))
				while (1); // catch errror!
		}			

		osMutexAcquire(Voice_mutex, osWaitForever);
		for (; i<NUM_WAVEFORM_SAMPLES; i++) {
			sum = 0;
		...
		if (i == 10) {
			...
			SoundBuffer[write_buffer_num][i] = sum; 
		}
...
const float startup_chord[] = {659.3, 493.9, 329.6, 246.9};
 ...
 void Thread_Sequencer(void) {
	...
	while (1) {
		...
		os_evt_set(EV_REFILL_SOUND, t_Refill_SoundBuffer);	
		Play_Waveform_with_DMA();
	}
}
#endif
